...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-background/85 border-b border-gold-deep/30 shadow-xs backdrop-blur-md">
      ...
      {/* Mobile nav */}
      {mobileOpen && (
        <nav className="md:hidden border-t border-gold-deep/20 px-4 pb-6 shadow-lg absolute w-full left-0 z-50 bg-parchment" aria-label="Mobile navigation">
          ...
          {navItems.map((item) => (
            <Link
              ...
              className="block py-3 text-[10px] font-sans font-extrabold uppercase tracking-[0.2em] text-ink hover:text-crimson border-b border-gold-deep/10 last:border-b-0"
            >
              {item.label}
            </Link>
          ))}
          <a
            href="tel:0914123456"
            className="mt-4 flex items-center justify-center gap-2 bg-crimson text-gold-soft px-4 py-2.5 rounded-lg text-[10px] font-sans font-bold uppercase tracking-[0.2em] border border-gold-deep"
          >
            <Phone className="h-4 w-4" />
            0914.123.456
          </a>
